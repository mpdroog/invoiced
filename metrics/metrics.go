...
func Dashboard(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	_, e := db.List(paths, db.Pagination{From:i, Count:count}, &u, func(filename, filepath, path string) error {
	...
	h := new(hour.Hour)
	_, e = db.List(paths, db.Pagination{From:i, Count:count}, h, func(filename, filepath, path string) error {
