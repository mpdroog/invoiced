...
// Package embed provides embedded assets for invoice templates.
package embed
...
// maxDecompressedSize limits decompressed asset size to prevent DoS
...
func bindataRead(data []byte, name string) ([]byte, error) {
	...
	_, err = io.Copy(&buf, io.LimitReader(gz, maxDecompressedSize))
	clErr := gz.Close()
...
func embedInvoiceXML() (*asset, error) {
	...
	info := bindataFileInfo{name: "embed/invoice.xml", size: 5336, mode: os.FileMode(0420), modTime: time.Unix(1528300192, 0)}
...
func Asset(name string) ([]byte, error) {
	...
	if f, ok := _bindata[cannonicalName]; ok {
		a, err := f()
		if err != nil {
			return nil, fmt.Errorf("asset %s can't read: %w", name, err)
		}
		return a.bytes, nil
	}
...
func AssetInfo(name string) (os.FileInfo, error) {
	...
	if f, ok := _bindata[cannonicalName]; ok {
		a, err := f()
		if err != nil {
			return nil, fmt.Errorf("assetInfo %s can't read: %w", name, err)
		}
		return a.info, nil
	}
...
var _bintree = &bintree{nil, map[string]*bintree{
	"embed": {nil, map[string]*bintree{
		"invoice.xml": {embedInvoiceXML, map[string]*bintree{}},
	}},
}}
