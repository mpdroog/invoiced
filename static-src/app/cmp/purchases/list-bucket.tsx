...
export default class PurchaseInvoices extends React.Component<IProps, IState> {
  ...
  private getSortValue(inv: PurchaseInvoice, field: string): string | number {
    switch (field) {
      ...
      case "Supplier":
        return inv.Supplier.Name !== "" ? inv.Supplier.Name : "";
      case "Amount": {
        const val = parseFloat(inv.TotalInc);
        return Number.isNaN(val) ? 0 : val;
      }
      case "Duedate":
        return inv.Duedate !== "" ? inv.Duedate : "";
      default:
        return "";
    }
  }
  ...
  private sortInvoices(
    invoices: { key: string; inv: PurchaseInvoice; bucket: string }[]
  ): { key: string; inv: PurchaseInvoice; bucket: string }[] {
  ...
  private sortHeader(field: string): React.JSX.Element {
    ...
    return (
      <th className="sortable" onClick={() => this.toggleSort(field)}>
        {field}
        {icon}
      </th>
    );
  ...
  private uploadXML(e: React.ChangeEvent<HTMLInputElement>): void {
    ...
    Axios.post("/api/v1/purchase/" + this.props.entity + "/" + this.props.year, form, {
  ...
  render(): React.JSX.Element {
    ...
    for (const dir in this.props.items) {
      ...
      items.forEach((inv: PurchaseInvoice) => {
        ...
        const key =
          inv.ID !== ""
            ? inv.Supplier.Name.toLowerCase().replace(/[^a-z0-9]/g, "-") +
              "-" +
              inv.ID.toLowerCase().replace(/[^a-z0-9]/g, "-")
            : dir;
        ...
        <tr key={key}>
          ...
          <td>
            <a
              ...
              href={`/api/v1/purchase/${this.props.entity}/${this.props.year}/${bucket}/${key}/pdf`}
              target="_blank"
              rel="noreferrer"
            >
            ...
            {isUnpaid && (
              <ActionLink className="btn btn-success" onClick={() => this.setPaid(key, bucket)}>
    ...
    if (res.length === 0) {
      res.push(
        <tr key="empty">
          <td colSpan={5}>No purchase invoices yet</td>
        </tr>
      );
    }
          ...
          <div className="card-body">
            ...
            <table className="table table-striped">
              <thead>
                <tr>
                  {this.sortHeader("ID")}
                  {this.sortHeader("Supplier")}
                  {this.sortHeader("Amount")}
...
class AddLineModal extends React.Component<IAddLineModalProps, IAddLineModalState> {
  ...
  componentDidMount(): void {
    ...
    Axios.get<InvoicesResponse>("/api/v1/invoices/" + this.props.entity + "/" + this.props.year, {
      params: { from: 0, count: 0 },
    })
      .then((res) => {
        ...
        <div className="modal-dialog modal-lg">
          <div className="modal-content">
            ...
            <div className="modal-body">
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) => this.setState({ selectedConcept: e.target.value })}
                >
                      ...
                      <tr key={idx}>
                        <td>
                          <input
                            type="radio"
                            name="selectedLine"
                            checked={this.state.selectedLine === idx}
                            onChange={() => this.setState({ selectedLine: idx })}
                          />
                        </td>
            ...
            <div className="modal-footer">
              <button type="button" onClick={this.props.onClose} className="btn btn-secondary">
                Cancel
              </button>
              <ActionButton onClick={() => this.addLine()} className="btn btn-primary">
                Add Line
              </ActionButton>
            </div>
          </div>
