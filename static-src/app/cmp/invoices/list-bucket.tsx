...
export default class Invoices extends React.Component<IInvoiceListProps, IInvoiceListState> {
  ...
  private getSelectedSum(invoiceList: { key: string; inv: Invoice }[]): number {
    let sum = 0;
    for (const { key, inv } of invoiceList) {
      if (this.state.selectedKeys.has(key)) {
        const val = parseFloat(inv.Total.Total);
        if (!Number.isNaN(val)) {
          sum += val;
        }
      }
  ...
  private getSortValue(inv: Invoice, field: string): string | number {
    switch (field) {
      case "Invoice":
        return inv.Meta.Invoiceid !== "" ? inv.Meta.Invoiceid : inv.Meta.Conceptid;
      case "Customer":
  ...
  private sortInvoices(
    invoices: { key: string; inv: Invoice; bucket: string }[]
  ): { key: string; inv: Invoice; bucket: string }[] {
  ...
  private conceptLine(key: string, inv: Invoice, bucket: string, isPending: boolean): React.JSX.Element {
      ...
      <tr key={key} className={isSelected ? "table-active" : ""}>
        <td>
          <input
            ...
            checked={isSelected}
            onChange={() => this.toggleSelect(key)}
          />
        </td>
        <td className="d-none d-md-table-cell">{key}</td>
        <td>{meta.Invoiceid}</td>
        <td>
          <span className="d-inline-block text-truncate-sm" title={customer.Name}>
            {customer.Name}
          </span>
        </td>
        ...
        <td className="text-end">
          <div className="btn-group">
            <a
              ...
              href={"#" + this.props.entity + "/" + this.props.year + "/" + "invoices/edit/" + bucket + "/" + key}
            >
            ...
            <ActionLink
              ...
              data-id={key}
              data-bucket={bucket}
              onClick={this.setPaid.bind(this)}
            >
  ...
  private finishedLine(key: string, inv: Invoice, bucket: string): React.JSX.Element {
    const isSelected = this.state.selectedKeys.has(key);
    return (
      <tr key={key} className={isSelected ? "table-active" : ""}>
        <td>
        ...
        <td>
          <span className="d-inline-block text-truncate-sm" title={inv.Customer.Name}>
            {inv.Customer.Name}
          </span>
        </td>
        ...
        <td className="text-end">
          <div className="btn-group">
            ...
            <a
              className="btn btn-primary"
              href={"/api/v1/invoice/" + this.props.entity + "/" + this.props.year + "/" + bucket + "/" + key + "/pdf"}
            >
  ...
  private uploadBalance(e: React.ChangeEvent<HTMLInputElement>): void {
    ...
    Axios.post("/api/v1/invoice-balance/" + this.props.entity + "/" + this.props.year, form, {
  ...
  private sortHeader(field: string, extraClass?: string): React.JSX.Element {
    ...
    return (
      <th className={className} onClick={() => this.toggleSort(field)}>
        {field}
        {icon}
      </th>
    );
  ...
  render(): React.JSX.Element {
    ...
    let invoiceList: { key: string; inv: Invoice; bucket: string }[] = [];
    ...
    if (this.props.bucket === "concepts") {
      headerButtons = (
        <div>
          <a
            id="js-new"
            href={"#" + this.props.entity + "/" + this.props.year + "/" + "invoices/add"}
    ...
    if (this.props.bucket === "sales-invoices-unpaid") {
      headerButtons = (
        <div>
          <a id="js-balance" onClick={this.openUpload.bind(this)} className="btn btn-primary showhide">
    ...
    const balanceUpload = (
      <form className="d-none" method="post" encType="multipart/form-data" action={url}>
        ...
        <div className="card">
          <div className="card-header d-flex align-items-center position-sticky sticky-card-header">
            ...
            <div className="ms-auto">
              <div className="btn-group nm7">{headerButtons}</div>
            </div>
          </div>
                  ...
                  <tr>
                    <th>
                      <input
                        ...
                        className="form-check-input"
                        checked={allSelected}
                        onChange={() => this.toggleSelectAll(allKeys)}
                        title="Select all"
                      />
                    </th>
                    <th className="d-none d-md-table-cell">#</th>
                    {this.sortHeader("Invoice")}
                    {this.sortHeader("Customer")}
                    {this.sortHeader("Amount", "text-end")}
                    {this.sortHeader("Duedate")}
